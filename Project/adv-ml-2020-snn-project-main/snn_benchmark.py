...
from bindsnet.encoding import PoissonEncoder, BernoulliEncoder, RankOrderEncoder, RepeatEncoder, SingleEncoder
from bindsnet.learning import PostPre, WeightDependentPostPre, Hebbian, NoOp
...
from models.snn_models import IF_Network, LIF_Network, SRM0_Network, DiehlAndCook_Network, AdaptiveLIFNodes_Network, CurrentLIFNodes_Network
...
parser.add_argument("--neuron_model", type=str, default="DiehlAndCook")

# -- update_rule specifies the type of learning rule (PostPre, WeightDependentPostPre, or Hebbian)
parser.add_argument("--update_rule", type=str, default="Hebbian")
...
# ADD BY SELF
elif args.neuron_model == "AdaptiveLIFNodes":
    network = AdaptiveLIFNodes_Network(n_inputs=784,update_rule=update_rule,input_shape=(1, 28, 28),batch_size=batch_size)
# ADD BY SELF
elif args.neuron_model == "CurrentLIFNodes":
    network = CurrentLIFNodes_Network(n_inputs=784,update_rule=update_rule,input_shape=(1, 28, 28),batch_size=batch_size)
...
for step, batch in enumerate(train_dataloader):
    ...
    s = output_spikes_monitor.get("s").permute((1, 0, 2)).cuda()
...
for step, batch in enumerate(test_dataloader):
    ...
    # get the spikes produced by the current batch
    spike_record = output_spikes_monitor.get("s").permute((1, 0, 2)).cuda()
